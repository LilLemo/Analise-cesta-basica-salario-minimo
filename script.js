...
async function mostrarResultados() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Erro ao chamar a API do servidor: ${response.statusText}`);
        }
        ...
        const nomeVencedor = dados.vencedor.split(':')[1].trim().toLowerCase();
        ...
        document.getElementById('vencedor-imagem').src = imagensPresidentes[nomeVencedor];
...
function desenharGraficoPizza(canvasId, dados, sufixo) {
    ...
    graficos[`pizza-${sufixo}`] = new Chart(ctx, {
        ...
        data: { labels: dados.labels, datasets: [{
            ...
            borderColor: ['#e94560', '#0f3460'], borderWidth: 2
        }]},
        options: { responsive: true, maintainAspectRatio: false,
...
function reiniciar() {
    ...
    telaBoasVindas.classList.add('ativa');
}
...
// --- Adiciona a lógica do Modal da Ficha Técnica ---
